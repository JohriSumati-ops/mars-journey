...
export default function Launch({ scrollerRef }) {
  ...
  useEffect(() => {
    ...
    // { passive: false } must be on the element itself (not window)
    // to avoid conflicting with R3F canvas passive listeners
  ...
  useEffect(() => {
    ...
    // Use a small timeout so the DOM is fully painted before ScrollTrigger
    // tries to measure elements — prevents the "Cannot read properties of
    // undefined (reading 'bind')" crash caused by measuring unmounted nodes
